...
When `readOnlyRootFilesystem` is enabled for broker, proxy, bookkeeper,
autorecovery, or ZooKeeper, the chart applies
`global.initCopyConfig.securityContext` and `global.initCopyConfig.resources` to
the `init-copy-config` container generated by sn-operator. By default they
inherit the component's security context and resources; both can be overridden
in values.

TLS Secret volumes use `tls.secretDefaultMode`, which defaults to decimal `288`
(octal `0440`). Set it to another Kubernetes file mode, or to `null` to leave
`defaultMode` unset.
